...
{
	ESP8266_GPIO_Config (); //片选和复位引脚
	
	ESP8266_USART_Config (); //mcu与esp通信的串口3
	
	
	macESP8266_RST_HIGH_LEVEL();	//复位引脚拉高

	macESP8266_CH_DISABLE();	//片选失效
...
static void ESP8266_USART_Config ( void )
{
	...
	/* config USART clock */
	macESP8266_USART_APBxClock_FUN ( macESP8266_USART_CLK, ENABLE );//RCC_APB1Periph_USART3
	macESP8266_USART_GPIO_APBxClock_FUN ( macESP8266_USART_GPIO_CLK, ENABLE );//RCC_APB2Periph_GPIOB  
	
	/* USART GPIO config */
	/* Configure USART Tx as alternate function push-pull */
	GPIO_InitStructure.GPIO_Pin =  macESP8266_USART_TX_PIN;//GPIO_Pin_10
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(macESP8266_USART_TX_PORT, &GPIO_InitStructure);  
  
	/* Configure USART Rx as input floating */
	GPIO_InitStructure.GPIO_Pin = macESP8266_USART_RX_PIN;//GPIO_Pin_11
	...
	ESP8266_USART_NVIC_Configuration ();	//串口中断配置
	
	
	USART_Cmd(macESP8266_USARTx, ENABLE);	//串口使能
...
void ESP8266_AT_Test ( void )
{
	...
	macESP8266_RST_HIGH_LEVEL();	//高电平使能
	Delay_ms ( 1000 );
	
	//发送AT，若返回OK则函数返回，否则重复十次，
	while ( count < 10 )
	{
